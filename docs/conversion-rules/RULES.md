...
## R-045: 어노테이션 값으로 다른 어노테이션을 넣을 때 `@` 없이 생성자 호출

- **카테고리**: language-idiom
- **도입 강**: [27강](../learning-log/step-27.md)
- **적용 조건**: 자바에서 어노테이션 파라미터의 값으로 다른 어노테이션을 중첩해서 쓰는 경우 (`@Outer(inner = @Inner(...))`)
...
## R-065: `@Jvm*` 상호운용 어노테이션 제거 전 grep 감사 + 테스트 실증

- **카테고리**: interop
- **도입 강**: [58강](../learning-log/step-58.md)
- **적용 조건**: 메인 소스가 전부 코틀린으로 바뀌었고 자바 코드(주로 아직 안 옮긴 테스트)만 일부 남아있는 시점에, 과거 자바 호출부를 대비해 미리 붙여둔 `@JvmStatic`/`@JvmOverloads`/`@JvmRecord`를 정리할 때

### 절차

1. 자바 쪽에서 해당 심볼(클래스명, 메서드 경로 등)을 실제로 참조하는지 `grep -rn "<심볼>" back/src --include="*.java"`로 전수 확인한다. "아마 안 쓸 것 같다"는 추측으로 지우지 않는다.
2. 자바 참조가 하나라도 있으면 그 어노테이션은 유지한다(같은 파일 안에서도 메서드별로 유지/제거가 갈릴 수 있음 — 예: `Ut.jwt.*`는 유지, `Ut.json.toString`/`Ut.cmd.runAsync`는 제거).
3. 제거 후에는 반드시 `./gradlew compileKotlin compileJava compileTestKotlin compileTestJava test`로 전체 빌드+테스트를 실행해 실제로 깨지는 곳이 없는지 실증적으로 재확인한다. 특히 Jackson 직렬화처럼 "컴파일은 되지만 런타임 동작이 바뀔 수 있는" 변경(예: `@JvmRecord` 제거로 접근자가 record 스타일 → JavaBean 게터로 바뀌는 경우, [R-030](#r-030-jsonignore의-use-site-target은-클래스-종류에-따라-갈린다)의 `@field:`/`@get:` 구분)은 grep만으로 안심하지 말고 관련 MockMvc 테스트가 실제로 통과하는지까지 확인한다.
